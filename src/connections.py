...
def consulta_cnpj(cnpj: str):
    ...
    # Caso a requisição não seja bem-sucedida, uma exceção será lançada
        ...
        if response.status_code == 404:
            print(f"Erro na requisição: {response.status_code} - CNPJ não encontrado.")
            raise ValueError("CNPJ não encontrado.")

        elif response.status_code == 429:
            print(f"Erro na requisição: {response.status_code} - Limite de requisições atingido.")
            raise ValueError("Limite de requisições atingido. Tente novamente mais tarde.")
        else:
